...
def build_lstm(input_size, hidden_size, num_layers, bias=True,
               batch_first=False, dropout: (bool, float) = False, bidirectional=False):
    ...
    args = [LSTMCell, input_size, hidden_size]
    args_other = [LSTMCell, hidden_size, hidden_size]
    if bidirectional:
        ...
        args = args + ([dropout] if not isinstance(dropout, bool) else [])
        args_other = args_other + ([dropout] if not isinstance(dropout, bool) else [])
    else:
        stack_type = StackedLSTM
        layer_type = LSTMLayer

    return stack_type(num_layers, layer_type, first_layer_args=args, other_layer_args=args_other)


def build_ln_lstm(input_size, hidden_size, num_layers, bias=True,
...
def test_script_rnn_layer(seq_len=5, batch=2, input_size=3, hidden_size=7):
...
def test_script_stacked_rnn(seq_len=5, batch=2, input_size=3, hidden_size=7, num_layers=4):
    inp = torch.randn(seq_len, batch, input_size)
    states = [LSTMState(torch.randn(batch, hidden_size),
                        torch.randn(batch, hidden_size))
              for _ in range(num_layers)]
    rnn = build_lstm(input_size, hidden_size, num_layers)
...
def test_script_stacked_bidir_rnn(seq_len=5, batch=2, input_size=3, hidden_size=7, num_layers=4):
    inp = torch.randn(seq_len, batch, input_size)
    states = [[LSTMState(torch.randn(batch, hidden_size),
                         torch.randn(batch, hidden_size))
               for _ in range(2)]
              for _ in range(num_layers)]
    rnn = build_lstm(input_size, hidden_size, num_layers, bidirectional=True)
    out, out_state = rnn(inp, states)
...
def test_script_stacked_lstm_dropout(seq_len=5, batch=2, input_size=3, hidden_size=7, num_layers=4):
    inp = torch.randn(seq_len, batch, input_size)
    states = [LSTMState(torch.randn(batch, hidden_size),
                        torch.randn(batch, hidden_size))
              for _ in range(num_layers)]
    rnn = build_lstm(input_size, hidden_size, num_layers, dropout=True)

    # just a smoke test
    out, out_state = rnn(inp, states)


def test_script_stacked_ln_lstm(seq_len=5, batch=2, input_size=3, hidden_size=7, num_layers=4):
    inp = torch.randn(seq_len, batch, input_size)
    states = [LSTMState(torch.randn(batch, hidden_size),
                        torch.randn(batch, hidden_size))
              for _ in range(num_layers)]
    rnn = build_ln_lstm(input_size, hidden_size, num_layers)
